...
class App {
    static showOrHideSectionContent(id) {
    ...
    static updateElementStyle(event) {
    ...
    static updateOtherPropertyGrid(elName, val) {
...
class Handlers {
    ...
    static onInputKeyPress(event) {
        if (event.keyCode == 13) {
            app_1.App.updateElementStyle(event);
        }
    }
    static onUpdateElementStyle(event) {
        app_1.App.updateElementStyle(event);
    }
    static updateElementStyle(event) {
        let elName = '#' + event.target.id;
        ...
        let attr = $(el).attr('cssName');
        let val = el.val();
        $(app_1.App.selectedElement).css(attr, val);
        app_1.App.updateSource();
        app_1.App.updateOtherPropertyGrid(elName, val);
    }
    static showVerticalLayout() {
        app_1.App.copy('h', 'v');
        $('#__verticalView').removeClass('__hidden');
        $('#__horizontalView').addClass('__hidden');
        app_1.App.currentEditor = '#__editorv';
        app_1.App.currentPreview = '#__previewv';
    }
    static showHorizontalLayout() {
        app_1.App.copy('v', 'h');
        $('#__verticalView').addClass('__hidden');
        $('#__horizontalView').removeClass('__hidden');
        app_1.App.currentEditor = '#__editorh';
        app_1.App.currentPreview = '#__previewh';
    }
    static clearEditors() {
        $('#__editorh').val('');
        $('#__editorv').val('');
    ...
    static save() {
        let editor = $(app_1.App.currentEditor);
        let text = editor.val();
        app_1.App.download('content.html', text);
    }
    static generateTOC() {
        let foo = 1;
        console.log(foo);
...
function setupSourcePropertiesContainer() {
    ...
    sectionNames.forEach(n => {
        ...
        app_1.App.sectionContentVisible['.' + contentName] = true;
    });
...
function wireUpSectionEvents(sectionNameClasses) {
    sectionNameClasses.forEach(sni => {
        ...
        $(document).on('click', sni.section, () => app_1.App.showOrHideSectionContent(sni.content));
    });
}
